...
import { Math as Math$1, Display, WEBGL, Scale, Game } from 'phaser3';
                    ...
                    (nameA == "star" && nameB == "player")) //
                {
                    ...
                    setTimeout(() => {
                        ...
                        if (self.collectedNumberOfStars == self.initialNumberOfStars) {
                            ...
                            const x = (self.player.x < 400) ? Math$1.Between(400, 800) : Math$1.Between(0, 400);
...
async function create() {
    ...
    this.add.text(100, 100, 'Box2D-WASM', {
        fontSize: '100px',
        fill: '#fff'
    });

    this.add.text(150, 280, 'Phaser 3', {
        fontSize: '100px',
        fill: '#fff'
    });
    ...
    contactListener = new ContactListener(metaData, scoreText, stars.length,
...
function update() {
    ...
    for (let i = 0; i < bombFixtures.length; i++) {
        const bombUserData = metaData[getPointer(bombFixtures[i])];
        bombUserData.bomb.x = bombFixtures[i].GetBody().GetPosition().x * this.pixelsPerMeter;
        bombUserData.bomb.y = bombFixtures[i].GetBody().GetPosition().y * this.pixelsPerMeter;
    }
